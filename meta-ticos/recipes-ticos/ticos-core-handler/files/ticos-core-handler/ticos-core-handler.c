...
#define SOCKET_PATH "/tmp/ticos-ipc.sock"
...
static int fd = -1;
char resp_socket[] = "/tmp/ticos-core-handler-XXXXXX.sock";
  ...
  *(int *)CMSG_DATA(cmsg) = STDIN_FILENO;
  ...
  while (true) {
    if (sendmsg(fd, &msg, 0) == -1) {
      // Wait up to tx_retry_count times for ticosd to restart
      if (++i > tx_retry_count) {
        fprintf(stderr, "Failed to sendmsg() to ticosd : %s\n",
                strerror(errno));
        goto cleanup;
      }
      sleep(1);
  ...
  if (setsockopt(fd, SOL_SOCKET, SO_RCVTIMEO, (const char *)&tv, sizeof tv) ==
    ...
    fprintf(stderr,
            ...
            "timeout if ticosd crashes : %s\n",
            strerror(errno));
  }

  if (recv(fd, buf, TX_BUFFER_SIZE, 0) != sizeof(int)) {
    fprintf(stderr, "Failed to recv() valid exit-code from ticosd : %s\n",
            strerror(errno));
    goto cleanup;
  }
  ret = (int)*buf;
  if (ret != EXIT_SUCCESS) {
    fprintf(stderr, "ticosd returned unexpected exit-code %d\n", ret);
  }
