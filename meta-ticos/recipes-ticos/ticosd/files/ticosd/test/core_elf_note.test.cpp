...
TEST(TestGroup_ElfNotes, Test_NoteWriting) {
  ...
  for (auto &s_case : s_cases) {
    ...
    uint8_t *const expected_buffer_contents =
      ticos_hex2bin(s_case.expected_buffer_contents_hex, &expected_buffer_size);

    const size_t buffer_size =
      ticos_core_elf_note_calculate_size(s_case.owner_name, s_case.description_size);
    ...
    uint8_t *const desc_buffer = ticos_core_elf_note_init(
