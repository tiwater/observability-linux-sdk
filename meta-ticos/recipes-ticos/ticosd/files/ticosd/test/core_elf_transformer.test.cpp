...
#include "coredump/core_elf_transformer.h"
...
TEST_GROUP(TestGroup_Transform) {
  ...
  sTicosCoreElfReadMemoryIO reader_io;
  sTicosCoreElfWriteMemoryIO writer_io;
  ...
  bool transform_elf(const Elf_Ehdr *buffer, size_t buffer_size) {
    ...
    ticos_core_elf_read_memory_io_init(&reader_io, elf_input_buffer, buffer_size);
    ticos_core_elf_write_memory_io_init(&writer_io, elf_output_buffer,
                                           sizeof(elf_output_buffer));

    ticos_core_elf_transformer_init(&transformer, &reader_io.io, &writer_io.io, &metadata,
                                       &transformer_handler);
    return ticos_core_elf_transformer_run(&transformer);
  }

  static ssize_t copy_proc_mem(sTicosCoreElfTransformerHandler * handler, Elf_Addr vaddr,
  ...
  bool written_ticos_metadata_note() const {
    auto *output_elf_header = (Elf_Ehdr *)elf_output_buffer;
 ...
 * but the Ticos metadata note is added otherwise.
 */
TEST(TestGroup_Transform, Test_UnexpectedSegmentType) {
  ...
  *segment_header = (Elf_Phdr){
  ...
  // Only 1 NOTE segment (Ticos metadata) got emitted:
  CHECK_EQUAL(1, written_num_segments());
  CHECK_TRUE(written_ticos_metadata_note());
}
...
TEST(TestGroup_Transform, Test_CopyNoteSegmentVerbatim) {
  ...
  *segment_header = (Elf_Phdr){
  ...
  // Original NOTE and Ticos metadata NOTE should get emitted:
...
TEST(TestGroup_Transform, Test_CopyLoadSegmentUsingCallback) {
  // Using more than TICOS_CORE_ELF_TRANSFORMER_PROC_MEM_COPY_BUFFER_SIZE_BYTES to exercise
  // copying in multiple chunks:
  const size_t data_size = 2 * TICOS_CORE_ELF_TRANSFORMER_PROC_MEM_COPY_BUFFER_SIZE_BYTES;
