...
TEST(TestGroup_ElfIsValid, Test_NullBuffer) {
  CHECK_FALSE(ticos_core_elf_reader_is_valid_core_elf(NULL, 1024));
}

TEST(TestGroup_ElfIsValid, Test_InputTooSmall) {
  uint8_t buffer[] = "\177ELF";
  CHECK_FALSE(ticos_core_elf_reader_is_valid_core_elf(buffer, sizeof(buffer)));
}
...
TEST(TestGroup_ElfIsValid, Test_InvalidMagic) {
  ...
  CHECK_FALSE(ticos_core_elf_reader_is_valid_core_elf(&elf_header, sizeof(elf_header)));
}
...
TEST(TestGroup_ElfIsValid, Test_Valid) {
  ...
  CHECK_TRUE(ticos_core_elf_reader_is_valid_core_elf(&elf_header, sizeof(elf_header)));
}
...
static void (*s_handle_segments_hook)(sTicosCoreElfReader *reader) = nullptr;

TEST_GROUP(TestGroup_ElfReaderReadAll) {
  sTicosCoreElfReader reader;
  sTicosCoreElfReadMemoryIO mio;
  ...
  static void handle_elf_header(sTicosCoreElfReader * reader, const Elf_Ehdr *elf_header) {
    mock()
  ...
  static void handle_segments(sTicosCoreElfReader * reader, const Elf_Phdr *segments,
                              size_t num_segments) {
    mock()
  ...
  static void handle_warning(sTicosCoreElfReader * reader, char *msg) {
    mock()
  ...
  static void handle_done(sTicosCoreElfReader * reader) {
  ...
  void setup_elf(const Elf_Ehdr *buffer, size_t buffer_size) {
    ...
    ticos_core_elf_read_memory_io_init(&mio, elf_buffer, buffer_size);
    ticos_core_elf_reader_init(&reader, &mio.io, &handler);
  }

  void test_read_segment_data_elf(void (*hook)(sTicosCoreElfReader *)) {
    ...
    *elf_header = s_core_elf_header_template;
    ...
    ticos_core_elf_reader_read_all(&reader);
  }
};
...
static void prv_past_stream_position_hook(sTicosCoreElfReader *reader) {
  uint8_t buffer[] = {0};
  const size_t expected_read_size = 0;
  CHECK_EQUAL(expected_read_size,
              ticos_core_elf_reader_read_segment_data(reader, 0, buffer, sizeof(buffer)));
}

/**
 * Tests that calling ticos_core_elf_reader_read_segment_data() with an at_pos that has already
...
static void prv_skip_to_position_hook(sTicosCoreElfReader *reader) {
  uint8_t buffer[2] = {0};
  // Skip over "AB":
  const size_t pos = sizeof(Elf_Ehdr) + 2;
  const size_t expected_read_size = 2;
  CHECK_EQUAL(expected_read_size,
              ticos_core_elf_reader_read_segment_data(reader, pos, buffer, sizeof(buffer)));
  MEMCMP_EQUAL("CD", buffer, sizeof(buffer));
}

/**
 * Tests that calling ticos_core_elf_reader_read_segment_data() with an at_pos that is past the
...
static void prv_skip_to_eof_hook(sTicosCoreElfReader *reader) {
  uint8_t buffer[1] = {0};
...
static void prv_read_until_hook(sTicosCoreElfReader *reader) {
  uint8_t buffer[10] = {0};
...
/**
 * Tests that calling ticos_core_elf_reader_read_segment_data() with a length that is past the
