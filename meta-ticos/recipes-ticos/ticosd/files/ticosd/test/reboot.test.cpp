...
static sTicosdTxData *g_stub_txdata = NULL;
...
bool ticos_reboot_is_untracked_boot_id(const char *last_tracked_boot_id_file,
                                          const char *current_boot_id) {
  return mock().actualCall("ticos_reboot_is_untracked_boot_id").returnBoolValue();
}

void ticos_reboot_process_pstore_files(char *pstore_dir) {
  mock().actualCall("ticos_reboot_process_pstore_files");
}

bool ticos_linux_boot_id_read(char boot_id[UUID_STR_LEN]) {
...
bool ticosd_txdata(sTicosd *ticosd, const sTicosdTxData *data, uint32_t payload_size) {
  g_stub_txdata = (sTicosdTxData *)malloc(sizeof(sTicosdTxData) + payload_size);
  memcpy(g_stub_txdata, data, sizeof(sTicosdTxData) + payload_size);

  return mock().actualCall("ticosd_txdata").returnBoolValue();
}
...
TEST_BASE(TicosdRebootUtest) {
  ...
  void expect_ticos_reboot_is_untracked_boot_id(bool is_untracked) {
    mock().expectOneCall("ticos_reboot_is_untracked_boot_id").andReturnValue(is_untracked);
  }
  ...
  void expect_ticosd_txdata_call(void) {
    mock().expectOneCall("ticosd_txdata").andReturnValue(true);
  }

  void expect_access_call(int val) { mock().expectOneCall("access").andReturnValue(val); }

  void expect_ticos_reboot_process_pstore_files_call(void) {
    mock().expectOneCall("ticos_reboot_process_pstore_files");
  }
  ...
  void expect_tx_unknown_reboot_reason() {
    ...
    expect_ticosd_txdata_call();                                  // tx data call
  }

  static void check_last_txdata_has_reboot_reason(int reboot_reason) {
    // Check object is valid
    json_object *payload, *object;
    CHECK_EQUAL(g_stub_txdata->type, kTicosdTxDataType_RebootEvent);
...
TEST_GROUP_BASE(TestGroup_Startup, TicosdRebootUtest){};
...
TEST(TestGroup_Startup, Test_DataCommsDisabled) {
  ...
  expect_ticos_reboot_is_untracked_boot_id(true);
  expect_ticos_reboot_process_pstore_files_call();

  bool success = ticosd_reboot_init(g_stub_ticosd, &fns);
...
TEST(TestGroup_Startup, Test_DataCommsEnabled) {
  ...
  check_last_txdata_has_reboot_reason(0);  // kTicosRebootReason_Unknown
...
TEST(TestGroup_Startup, Test_ValidRebootReasonFile) {
  ...
  check_last_txdata_has_reboot_reason(2);      // kTicosRebootReason_UserReset
  check_file_does_not_exist(tmp_reboot_file);  // File is deleted after having been read
...
TEST(TestGroup_Startup, Test_PstorePanic) {
  ...
  check_last_txdata_has_reboot_reason(0x8008);  // kTicosRebootReason_KernelPanic
...
TEST_GROUP_BASE(TestGroup_Shutdown, TicosdRebootUtest){};
...
TEST(TestGroup_Shutdown, Test_StillRunning) {
  ...
  ticosd_reboot_init(g_stub_ticosd, &fns);
...
TEST(TestGroup_Startup, Test_BootIdAlreadyTracked) {
  expect_enable_data_collection_get_boolean_call(true);  // collection enabled
  expect_last_tracked_boot_id_file_generate_call();
  expect_ticos_reboot_is_untracked_boot_id(false);
