...
TEST_GROUP(TicosMinimalCbor){void setup(){} void teardown(){}};
...
static void prv_run_unsigned_integer_encoder_check(uint32_t value, const uint8_t *expected_seq,
  ...
  ticos_cbor_encoder_init(&encoder, prv_write_cb, result, sizeof(result));

  const bool success = ticos_cbor_encode_unsigned_integer(&encoder, value);
  CHECK(success);

  const size_t encoded_length = ticos_cbor_encoder_deinit(&encoder);
  LONGS_EQUAL(expected_seq_len, encoded_length);

  MEMCMP_EQUAL(expected_seq, result, expected_seq_len);
}

TEST(TicosMinimalCbor, Test_EncodeUnsignedInt) {
...
static void prv_run_array_encoder_check(const uint32_t *values, size_t num_values,
  ...
  bool success = ticos_cbor_encode_array_begin(&encoder, num_values);
  CHECK(success);

  for (size_t i = 0; i < num_values; i++) {
    success = ticos_cbor_encode_unsigned_integer(&encoder, values[i]);
    CHECK(success);
  }
...
TEST(TicosMinimalCbor, Test_Join) {
  ...
  bool success = ticos_cbor_encode_array_begin(&encoder, TICOS_ARRAY_SIZE(values123));
  CHECK(success);

  // build list of integers in a seperate encoder context
  sTicosCborEncoder encoder2;
  char cbor_ints[3];
  ticos_cbor_encoder_init(&encoder2, prv_write_cb, cbor_ints, sizeof(cbor_ints));

  for (size_t i = 0; i < TICOS_ARRAY_SIZE(values123); i++) {
    success = ticos_cbor_encode_unsigned_integer(&encoder2, values123[i]);
    CHECK(success);
  }

  size_t encoded_length = ticos_cbor_encoder_deinit(&encoder2);
  LONGS_EQUAL(sizeof(cbor_ints), encoded_length);

  // join results from integer encoding into main encoder
  success = ticos_cbor_join(&encoder, cbor_ints, encoded_length);

  const uint8_t three_num_array_expected_encode[] = {0x83, 0x01, 0x02, 0x03};
  encoded_length = ticos_cbor_encoder_deinit(&encoder);
...
TEST(TicosMinimalCbor, Test_EncodeArray) {
  // RFC Appendix A. Examples
  ...
  const uint32_t values123[] = {0x1, 0x2, 0x3};
  const uint8_t three_num_array_expected_encode[] = {0x83, 0x01, 0x02, 0x03};
  prv_run_array_encoder_check(values123, TICOS_ARRAY_SIZE(values123),
  ...
  prv_run_array_encoder_check(longer_array, TICOS_ARRAY_SIZE(longer_array),
...
static void prv_run_signed_integer_encoder_check(int32_t value, const uint8_t *expected_seq,
  ...
  const bool success = ticos_cbor_encode_signed_integer(&encoder, value);
  CHECK(success);
...
TEST(TicosMinimalCbor, Test_EncodeSignedInt) {
  // RFC Appendix A.  Examples
...
static void prv_run_long_signed_integer_encoder_check(int64_t value, const uint8_t *expected_seq,
  ...
  const bool success = ticos_cbor_encode_long_signed_integer(&encoder, value);
  CHECK(success);
...
TEST(TicosMinimalCbor, Test_EncodeLongSignedInt) {
...
static void prv_run_string_encoder_check(const char *str, const uint8_t *expected_seq,
  ...
  const bool success = ticos_cbor_encode_string(&encoder, str);
  CHECK(success);
...
TEST(TicosMinimalCbor, Test_EncodeString) {
  // RFC Appendix A.  Examples
...
static void prv_run_incremental_string_encoder_check(const char *str, const uint8_t *expected_seq,
  ...
  const bool success = ticos_cbor_encode_string_begin(&encoder, strlen(str));
  CHECK(success);

  for (size_t i = 0; i < strlen(str); ++i) {
    CHECK(ticos_cbor_join(&encoder, &str[i], 1));
  }
...
TEST(TicosMinimalCbor, Test_EncodeStringIncremental) {
  // RFC Appendix A.  Examples
  const uint8_t expected_enc_empty[] = {0x60};
...
static void prv_run_binary_encoder_check(const void *buf, size_t buf_len,
  ...
  const bool success = ticos_cbor_encode_byte_string(&encoder, buf, buf_len);
  CHECK(success);
...
TEST(TicosMinimalCbor, Test_EncodeBinaryString) {
  // RFC Appendix A.  Examples
...
static void prv_run_uint64_as_double_encoder_check(double g, const uint8_t *expected_seq,
  ...
  const bool success = ticos_cbor_encode_uint64_as_double(&encoder, value);
  CHECK(success);
...
TEST(TicosMinimalCbor, Test_EncodeFLoatString) {
  // RFC Appendix A.  Examples
...
static void prv_encode_test_dictionary(sTicosCborEncoder *encoder) {
  // {
  //   "a": "A",
  //   "b": "B",
  //   "c": "C",
  ...
  //   "e": "E"
  // }
  bool success = ticos_cbor_encode_dictionary_begin(encoder, 5);
  CHECK(success);

  const char *enc[] = {"a", "b", "c", "d", "e"};
  for (size_t i = 0; i < TICOS_ARRAY_SIZE(enc); i++) {
    char value[2] = {0};
    value[0] = *enc[i] - 32;  // get capital representation

    success = ticos_cbor_encode_string(encoder, enc[i]);
    CHECK(success);
    success = ticos_cbor_encode_string(encoder, value);
    CHECK(success);
  }
}

// Real world encoding of a set of data
TEST(TicosMinimalCbor, Test_EncodeDictionary) {
  // first let's just compute the size
  sTicosCborEncoder encoder;
  ticos_cbor_encoder_size_only_init(&encoder);
  prv_encode_test_dictionary(&encoder);
  const size_t size_needed = ticos_cbor_encoder_deinit(&encoder);
...
// nested dictionary
TEST(TicosMinimalCbor, Test_EncodeNestedDictionary) {
  // {
  //   "a": {
  //     "b": "c"
  //    }
  ...
  sTicosCborEncoder encoder;
  ticos_cbor_encoder_init(&encoder, prv_write_cb, result, sizeof(result));
  bool success = ticos_cbor_encode_dictionary_begin(&encoder, 1);
  CHECK(success);
  success = ticos_cbor_encode_string(&encoder, "a");
  CHECK(success);
  success = ticos_cbor_encode_dictionary_begin(&encoder, 1);
  CHECK(success);
  success = ticos_cbor_encode_string(&encoder, "b");
  CHECK(success);
  success = ticos_cbor_encode_string(&encoder, "c");
  CHECK(success);
...
TEST(TicosMinimalCbor, Test_BufTooSmall) {
  sTicosCborEncoder encoder;
  char result[1];
  ticos_cbor_encoder_init(&encoder, prv_write_cb, result, sizeof(result));

  // we only have a 1 byte result buffer so a 2 byte encoding should fail
  const bool success = ticos_cbor_encode_string(&encoder, "a");
  CHECK(!success);
}
