...
TEST(TestGroup_LastBootId, Test_OpenFileFailed) {
  CHECK_FALSE(ticos_reboot_is_untracked_boot_id("/", current_boot_id));
}

TEST(TestGroup_LastBootId, Test_LastTrackedBootIdFileNotExistingYet) {
  CHECK_TRUE(ticos_reboot_is_untracked_boot_id(last_tracked_boot_id_file, current_boot_id));
...
TEST(TestGroup_LastBootId, Test_BootIdAlreadyTracked) {
  ...
  CHECK_FALSE(ticos_reboot_is_untracked_boot_id(last_tracked_boot_id_file, current_boot_id));
}
...
TEST(TestGroup_LastBootId, Test_ReadBack) {
  ...
  for (unsigned int i = 0; i < TICOS_ARRAY_SIZE(boot_ids); ++i) {
    CHECK_TRUE(ticos_reboot_is_untracked_boot_id(last_tracked_boot_id_file, boot_ids[i]));
