...
#include "../src/ticosctl/parse_attributes.h"
...
TEST(TestParseAttributesGroup, BasicStrings) {
  ...
  CHECK(ticosd_parse_attributes(argv, TICOS_ARRAY_SIZE(argv), &json));
...
TEST(TestParseAttributesGroup, EmptyAttributes) {
  ...
  CHECK(ticosd_parse_attributes(argv, TICOS_ARRAY_SIZE(argv), &json) == false);
