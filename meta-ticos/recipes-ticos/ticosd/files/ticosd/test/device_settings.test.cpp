...
/**
 * @brief Success test case, ticos-device-info is successfully callable and returns all expected
 * values
 *
 */
TEST(TestDeviceSettingsGroup, SuccessTest) {
  ...
  static const char* successTest_fgetsArray[] = {"TICOS_DEVICE_ID=device_id",
                                                 "TICOS_HARDWARE_VERSION=hardware_version"};

  current_fgetsArray = successTest_fgetsArray;
  current_fgetsArray_len = sizeof(successTest_fgetsArray) / sizeof(char*);

  sTicosdDeviceSettings* settings = ticosd_device_settings_init();
  CHECK(settings != NULL);

  STRCMP_EQUAL(settings->device_id, "device_id");
  STRCMP_EQUAL(settings->hardware_version, "hardware_version");

  ticosd_device_settings_destroy(settings);
}

/**
 * @brief Failure test case, ticos-device-info is successfully callable but does not return all
 * expected values
 *
 */
TEST(TestDeviceSettingsGroup, MissingEntryTest) {
  ...
  static const char* missingEntryTest_fgetsArray[] = {"TICOS_DEVICE_ID = device_id"};
...
/**
 * @brief Failure test case, ticos-device-info is not callable
 *
 */
