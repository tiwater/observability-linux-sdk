...
size_t ticos_core_elf_note_calculate_size(const char *owner_name, size_t description_size) {
  const size_t owner_name_size = prv_calc_owner_name_size(owner_name);
  return sizeof(Elf_Nhdr) + TICOS_ALIGN_UP(owner_name_size, 4) +
         TICOS_ALIGN_UP(description_size, 4);
}

uint8_t *ticos_core_elf_note_init(void *out_buffer, const char *owner_name,
  ...
  *(Elf_Nhdr *)out_buffer = (Elf_Nhdr){
  ...
  const size_t owner_name_and_padding_size = TICOS_ALIGN_UP(owner_name_size, 4);
  memset(payload, 0, owner_name_and_padding_size + TICOS_ALIGN_UP(description_size, 4));
